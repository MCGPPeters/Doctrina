namespace Doctrina.Math.Pure.Structure.Category

type Functor<'a> = Functor of 'a with

 
        static member inline map : ((^a -> _) -> Functor<'a> -> Functor<_>) = (^a: (static member inline map: (^a -> _) -> Functor<'a> -> Functor<_>) (f, x))



module Monoid =
    let inline empty () = (^a: (static member empty: ^a) ())

    let inline append (a:^a) (b:^a) = (^a: (static member append: ^a -> ^a -> ^a) (a, b))

    let inline concat a = (^a: (static member concat: ^a list -> ^a) (a))
