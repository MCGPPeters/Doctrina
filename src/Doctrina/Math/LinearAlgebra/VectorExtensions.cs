...
namespace Doctrina.Math.LinearAlgebra
{
    ...
    public static class VectorExtensions
    {
        ...
        public static Vector Apply(this Vector v, Func<double, double> f) => Vector.Calc(v, f);

        /// <summary>An IEnumerable&lt;Vector&gt; extension method that sums the given source.</summary>
        /// <param name="v">The v to act on.</param>
        /// <returns>A Vector.</returns>
        public static double Sum(this Vector v) => Vector.Sum(v);
        ...
        /// <param name="v"></param>
        /// <returns></returns>
        public static Vector Log(this Vector v) => Vector.Log(v);
        ...
        public static Vector Pow(this Vector v, double power) => Vector.Pow(v, power);

        /// <summary>A Vector extension method that products the given v.</summary>
        /// <param name="v">The v to act on.</param>
        /// <returns>A double.</returns>
        public static double Prod(this Vector v) => Vector.Prod(v);

        /// <summary>A Vector extension method that outers.</summary>
        /// <param name="x">The x to act on.</param>
        /// <param name="y">The Vector to process.</param>
        /// <returns>A Matrix.</returns>
        public static Matrix Outer(this Vector x, Vector y) => Vector.Outer(x, y);
        ...
        /// <param name="expand">
        ///     If True, a binary Vector is expanded to a m x 2 Matrix, otherwise a binary Matrix of m x 1 is
        ///     returned.
        /// </param>
        ...
        public static Matrix ToBinaryMatrix(this Vector v, bool expand = false)
        {
            ...
            var count = sorted.Count > 2 ? sorted.Count : expand ? sorted.Count : 1;

            var Y = Matrix.Zeros(v.Length, count);
        ...
        public static Vector ToVector(this double[] array) => new Vector(array);
        ...
        /// <returns>e as a Matrix.</returns>
        public static Matrix ToMatrix(this IEnumerable<double[]> e) => new Matrix(e.ToArray());

        /// <summary>
        ///     Reshapes the given Vector into a Matrix form given the specified dimension.
        ///     <para>
        ///         Reads from the source vector and repopulates from left to right when <paramref name="vectorType" /> equals
        ///         'Col' otherwise uses top down approach.
        ///     </para>
        /// </summary>
        /// <param name="source"></param>
        /// <param name="dimension">
        ///     If the <paramref name="vectorType" /> equals 'Col' the <paramref name="dimension" /> becomes
        ///     the column width otherwise it is row height.
        /// </param>
        ...
        /// <returns>Matrix.</returns>
        public static Matrix Reshape(this Vector source, int dimension, VectorType vectorType = VectorType.Col,
            VectorType fillType = VectorType.Row) => Matrix.Reshape(source, dimension, vectorType, fillType);

        /// <summary>A Vector extension method that diags.</summary>
        /// <param name="v">The v to act on.</param>
        /// <returns>A Matrix.</returns>
        public static Matrix Diag(this Vector v) => Vector.Diag(v);
        ...
        /// <returns>A Matrix.</returns>
        public static Matrix Diag(this Vector v, int n, int d) => Vector.Diag(v, n, d);

        /// <summary>A Vector extension method that normals.</summary>
        /// <param name="v">The v to act on.</param>
        /// <returns>A double.</returns>
        public static double Norm(this Vector v) => Vector.Norm(v, 2);

        /// <summary>A Vector extension method that normals.</summary>
        /// <param name="v">The v to act on.</param>
        /// <param name="p">The double to process.</param>
        /// <returns>A double.</returns>
        public static double Norm(this Vector v, double p) => Vector.Norm(v, p);
        ...
        public static double StdDev(this Vector source, bool isSamplePop = false) =>
            System.Math.Sqrt(source.Variance(isSamplePop));
        ...
        /// <param name="n">The int to process.</param>
        /// <returns>A Vector.</returns>
        public static Vector Expand(this Vector source, int n) => Vector.Combine(source, new Vector(n));

        /// <summary>A Vector extension method that expands.</summary>
        /// <param name="source">The source to act on.</param>
        /// <param name="s">The Vector to process.</param>
        /// <returns>A Vector.</returns>
        public static Vector Expand(this Vector source, Vector s) => Vector.Combine(source, s);
        ...
        {
            if (source.Length < n)
                throw new InvalidOperationException(string.Format("Cannot get top {0} from a {1} length vector", n,
                    source.Length));
        ...
        /// <returns>A Vector.</returns>
        public static Vector Round(this Vector v, int decimals = 0) => Vector.Round(v, decimals);
        ...
        public static Vector Slice(this Vector v, IEnumerable<int> indices, bool preserveOrder)
        {
            if (preserveOrder)
            {
                var vector = new Vector(indices.Count());
                var i = -1;
        ...
        public static Vector Slice(this Vector v, int fromIndex, int toIndex) =>
            v.Skip(fromIndex).Take(toIndex - fromIndex + 1).ToVector();
        ...
        /// <param name="x">The x to act on.</param>
        /// <returns>A double.</returns>
        public static double Dot(this Vector v, Vector x) => Vector.Dot(v, x);
        ...
        /// <returns></returns>
        public static double ToDouble(this Vector vector) => vector.First();
        ...
        public static bool ContainsNaN(this Vector vector) => Vector.ContainsNaN(vector);

        /// <summary>A Vector extension method that query if 'vector' is na n.</summary>
        /// <param name="vector">The vector to act on.</param>
        /// <returns>true if na n, false if not.</returns>
        public static bool IsNaN(this Vector vector) => Vector.IsNaN(vector);
        ...
        public static bool IsBinary(this Vector vector)
        {
            ...
            return v.Count() == 2 && v.Contains(1d) && (v.Contains(0d) || v.Contains(-1d)) ||
                   v.Count() == 1 && (v.Contains(1d) || v.Contains(0d) || v.Contains(-1d));
        }
        ...
        /// <returns></returns>
        public static Vector ToBinary(this Vector v, Func<double, bool> fnPredicate, double trueValue = 1.0,
        ...
        ///     <para>
        ///         When <paramref name="minValue" /> and <paramref name="maxValue" /> are null, the vector instance min and max
        ...
        /// <returns></returns>
        public static Vector Rescale(this Vector v, double min, double max, double? minValue = null,
            double? maxValue = null)
        {
            var min_tm1 = minValue ?? v.Min();
            var max_tm1 = maxValue ?? v.Max();

            var v_t = v.Each(d => (max - min) * (d - min_tm1) / (max_tm1 - min_tm1), true);
